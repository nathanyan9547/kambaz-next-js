...
export default function PeopleDetails({ uid, onClose }: { uid: string | null; onClose: () => void; }) {
  ...
  const saveUser = async () => {
    ...
    const updatedUser: User = { ...user, firstName, lastName, email, role };
  ...
  return (
    <div className="wd-people-details position-fixed top-0 end-0 bottom-0 bg-white p-4 shadow w-25">
      ...
      {editing && (
        <FormControl type="email" className="w-75 wd-edit-email mt-1"
      ...
      {editing && (
        <select className="form-select w-75 mt-1 wd-edit-role"
          value={role}
